...
def preprocess(image):
    ...
    return image[None, ...]
print("pass4")
def execute(img, src, people_num,id_position,clss): #delted t
    color = (0, 255, 0)
    id_position=[]
    time = []
    data = preprocess(img)
   ...
   #     if(human_partm == 1):
    ...
    for j in range(len(keypoints_agg)):
        if(keypoints_agg[j][11][1]):# right hip
            id=j
            x = round(keypoints_agg[j][11][2]*1280)
            y = round(keypoints_agg[j][11][1]*720)
            #if(i == people_num_tmp)                         
          #  id_position[id] = x
            cv2.circle(src, (x, y), 3, color, 2)
        if(keypoints_agg[j][9][1] or keypoints_agg[j][10][1]):
            if(keypoints_agg[j][9][1]):
                x1 = round(keypoints_agg[j][9][2]*1280)
                y1 = round(keypoints_agg[j][9][1]*720)
                id_position.append([x,x1])
               ...
               # else:
               #     time = 0
            elif(keypoints_agg[j][10][1]):
                x1 = round(keypoints_agg[j][10][2]*1280)
                y1 = round(keypoints_agg[j][10][1]*720)
                id_position.append([x,x1])
                cv2.circle(src, (x1,y1), 3, color, 2)
                print("wrist", x1, y1)
               # if(y < 400):
       ...
       # for j in range(len(keypoints)):
        #    if keypoints[j][1]:
        #        x = round(keypoints[j][2] * 1280)
        #        y = round(keypoints[j][1] * 720)
        #        cv2.circle(src, (x, y), 3, color, 2)
    ...
    #        else:
        #            cv2.putText(src , "%d" % int(keypoints[j][0]), (x + 5, y),  cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 1)
           ...
           # elif(j==0):
            ...
            #    cv2.putText(src, nos,(200,200), cv2.FONT_HERSHEY_SIMPLEX,1, (0,255,255),1)
            
    #added part#for writing key point
    #sort as x coordinate ascending
    #id_position = sorted(id_position.items(), key = lambda item:item[1])
    id_position = sorted(id_position, key=itemgetter(0))
    for i in range(len(clss)):
        if(round(clss[i]) == 1 and (id_position[i][1]<300 or id_position[i][1]> 800)):
            time.append(1)
        else:
            time.append(0)
...
def loop_and_detect(cam, trt_yolo, conf_th, vis):
    ...
    out_video = cv2.VideoWriter('/home/ee201511400/test_wrist_6.mp4', fourcc, 30.0 , (1280, 720))
    ...
    while True:
        ...
        people_num,id_position,time_for_disinfection  = execute(img_e, img, people_num,id_position,clss)
      #  if(time_for_disinfection > 1000):
      #      print("disinfected")
      #  else:
      #      print("disinfection required")
        ...
        img,x_min,y_min,x_max,y_max = vis.draw_bboxes(img, boxes, confs, clss)
